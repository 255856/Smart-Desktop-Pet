...
_STYLE_RULE = (
    ...
    "【多轮工具使用规则】按主人需要可连续调用多个工具以收集信息；"
    "当你认为已收集足够信息、可以直接给主人完整答案时，"
    "用一句自然中文把搜索/工具结果整理后告诉主人（**不要只说「让我给主人推荐」这种"
    "过渡话**——必须基于工具结果给出具体内容，如具体番名、价格、链接等）。"
)
...
class AgentLoop:
    ...
    async def run(
        ...
        for turn in range(self.max_turns):
            ...
            # 关键：不强制注入 user 指令。模型自主 ReAct —— 调完工具后自己决定
            # 是再调一个 / 还是出 final answer。如果连续多轮纯调工具无文字（陷入
            # 「调工具上瘾」），才在最后一轮前追加 user 总结提示，避免死循环。
            if consecutive_tool_only >= MAX_CONSECUTIVE_TOOL_ONLY_TURNS - 1:
                tool_summary = self._format_tool_results(messages)
                messages.append({
                    "role": "user",
                    "content": (
                        f"[提示] 你已经连续多轮调工具没出文字了，结果如下：\n{tool_summary}\n"
                        "**下一轮请不要再调任何工具**，直接用中文给主人一个完整、自然的总结。"
                    ),
                })
